...
class Trainer:
    ...
    def run_stage(self, stage_name, num_epochs, early_stopping=True):
        ...
        for epoch in range(num_epochs):
            ...
            # ==================== PRE-COMPUTE EMBEDDINGS ONCE PER EPOCH ====================
            with torch.no_grad():
            ...
            for batch in pbar:
                ...
                # L2 regularization on user preference only
                reg_loss = Config.weight_decay * (
                    self.model.v_gcn.preference.norm(2).pow(2) +
                    self.model.t_gcn.preference.norm(2).pow(2)
                )
